    ...
    useEffect(() => {
        ...
        for (let i = 1; i <= props.cantidadTotalPaginas; i++) {
            if (i >= props.paginaActual - props.radio && i <= props.paginaActual + props.radio) {
                links.push({
                    texto: `${i}`,
                    activo: props.paginaActual === 1,
                    habilitado: true, pagina: i
                })
            }
        }
        const paginaSigHabilitada = props.paginaActual !== props.cantidadTotalPaginas && props.cantidadTotalPaginas > 0;
        var paginaSiguiente = props.paginaActual + 1;
        links.push({
            texto: 'Siguiente',
            pagina: paginaSiguiente,
            habilitado: paginaSigHabilitada,
            activo: false
        })
